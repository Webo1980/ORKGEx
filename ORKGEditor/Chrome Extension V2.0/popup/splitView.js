...
async function fetchData(imageUrl) {
  ...
  // Define the headers using the apiKey
  const headers = {
  ...
  fetch(imageUrl)
    ...
    .then(blob => {
      ...
      reader.onloadend = () => {
        const base64EncodedUrl = reader.result.split(',')[1]; // Extract the base64 encoded data
          ...
          messages: [
              {
                  ...
                  content: [
                      {
                          type: "text",
                          text: `Act as a researcher and transform what you see in the image into subject-predicate-object form. Return the data in a JSON object, strictly in the following format: {
                            "data": [
                                {
                        ...
                        Ensure that each entry includes both the subject value and the predicate name. Do not srround the JSON object with any special characters.`
                      },
                      {
                          type: "image_url",
                          image_url: {
                              url: `data:image/jpeg;base64,${base64EncodedUrl}`
                          }
                      }
                  ]
              }
...
// Ensure event listeners are attached after rendering triples
/*function renderTriplesView(annotationData) {
  if (annotationData && annotationData.data) {
    console.log(annotationData);
    spinner.style.display = "none"; // hide the spinner
    ...
    // Modified annotation data starts as a deep copy of the original
    const modifiedAnnotationData = deepCopy(annotationData);
    console.log(modifiedAnnotationData);
